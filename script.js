...
// ─── Constants ────────────────────────────────────────────────────────────────
var CW = 390, CH = 720;
var RING_R = 48, RING_SW = 11;
var FLICK_MIN = 18;
// リングは画面下寄り（片手スワイプしやすい位置）
var SPAWN_X0 = 60, SPAWN_X1 = 330, SPAWN_Y0 = 380, SPAWN_Y1 = 560;
// ボーナスO・スプーン類は上エリア
var BONUS_Y0 = 100, BONUS_Y1 = 340;
var DIRS = ['up','down','left','right'];
var GAP_A = { up:-Math.PI/2, down:Math.PI/2, left:Math.PI, right:0 };
var CLR = {
  neon:'#00d4ff', neonG:'rgba(0,212,255,0.3)',
  gold:'#ffd700', goldG:'rgba(255,215,0,0.45)',
  danger:'#ff3366', dangerG:'rgba(255,51,102,0.55)',
  purple:'#7c3aed', purpleG:'rgba(124,58,237,0.3)',
  white:'#ffffff', textP:'#e8f4ff', textM:'rgba(180,220,255,0.6)',
  glassB:'rgba(0,212,255,0.3)', bg1:'#060810'
};
...
function resetGame() {
  rings=[]; bullets=[]; particles=[]; utensils=[]; popups=[]; bigMsgs=[]; missGhosts=[];
...
// ─── Miss Ghosts（失敗したCの霊が視界を遮る）───────────────────────────────
function addMissGhost(ring) {
  // ランダムな方向の大きいCの跡を追加
  ...
  missGhosts.push({
    ...
    dir: dir,
    size: 60 + Math.random()*80,     // 大きさランダム
    alpha: 0.18 + Math.random()*0.22,// 半透明
    rot: Math.random()*Math.PI*2,    // ランダム回転
    drift: (Math.random()-0.5)*0.3   // ゆっくり漂う
  });
}
...
function drawMissGhosts() {
  for (var i=0; i<missGhosts.length; i++) {
    ...
    ctx.lineWidth = g.size * 0.22;
    ctx.lineCap = 'round';
    ctx.shadowColor = CLR.danger;
    ctx.shadowBlur = 20;
    ctx.beginPath();
    ctx.arc(0, 0, g.size, sa, ea);
    ctx.stroke();
    ctx.restore();
  }
}
...
function makeRing(x, y, dir) {
  return {
    ...
    scale:0.04, targetScale:1,
    state:'alive', // alive|fading|closing|breaking|dead
    closeP:0, breakP:0, fadeP:0,
...
function updateRing(r, dt, t) {
  if (r.scale < r.targetScale) r.scale = Math.min(r.targetScale, r.scale + dt*2.2);
  if (r.state==='alive' && r.scale>=0.99)
    r.targetScale = 1 + Math.sin(t*2.5+r.depthPh)*0.025;
...
function drawRing(r) {
  ...
  var alpha;
  if (r.state==='breaking') alpha = Math.max(0, 1-r.breakP);
  else if (r.state==='fading') alpha = Math.max(0, 1-r.fadeP);
  else alpha = 1;
  ...
  if (r.missed) {
    // ミスしたリング → 赤く染まってフェードアウト
    drawMissedRing(r, glow);
  } else if (r.isBonus) {
    drawBonusRing(glow);
  } else {
    drawCRing(r, glow);
  }
  if (r.state==='breaking') drawShards(r.breakP, r.missed);
  ctx.restore();
}

function drawMissedRing(r, glow) {
  var gap = Math.PI * 0.44;
  var base = r.dir ? GAP_A[r.dir] : 0;
  var sa = base+gap, ea = base+Math.PI*2-gap;
  ctx.strokeStyle = 'rgba(255,51,102,0.8)';
  ctx.lineWidth = RING_SW+8;
  ctx.shadowColor = CLR.danger; ctx.shadowBlur = 28;
  ctx.lineCap = 'round';
  ctx.beginPath(); ctx.arc(0,0,RING_R,sa,ea); ctx.stroke();
  ctx.strokeStyle = '#ff8899'; ctx.lineWidth = RING_SW; ctx.shadowBlur = 8;
  ctx.beginPath(); ctx.arc(0,0,RING_R,sa,ea); ctx.stroke();
}
...
function drawCRing(r, glow) {
  ...
  ctx.lineWidth = RING_SW+12; ctx.shadowColor=CLR.neon; ctx.shadowBlur=22+glow*10;
  ctx.lineCap='round';
  ctx.beginPath(); ctx.arc(0,0,RING_R,sa,ea); ctx.stroke();
  // main
  ...
  ctx.beginPath(); ctx.arc(0,0,RING_R,sa,ea); ctx.stroke();
  // inner accent
  ctx.strokeStyle=CLR.neon; ctx.lineWidth=2.5; ctx.shadowBlur=0;
...
function drawArrow(dir, glow) {
  var a=GAP_A[dir], r1=RING_R+20, r2=RING_R+34;
  var ax=Math.cos(a), ay=Math.sin(a);
  ctx.save();
  ctx.globalAlpha *= (0.28+glow*0.32);
  ...
  ctx.lineTo(ax*r2+Math.cos(a+Math.PI+ha)*10, ay*r2+Math.sin(a+Math.PI+ha)*10);
  ctx.moveTo(ax*r2,ay*r2);
  ctx.lineTo(ax*r2+Math.cos(a+Math.PI-ha)*10, ay*r2+Math.sin(a+Math.PI-ha)*10);
  ctx.stroke();
  ctx.restore();
...
function drawBonusRing(glow) {
  var p=0.9+glow*0.13;
  // 外コロナ
  ctx.strokeStyle='rgba(255,215,0,'+(0.22+glow*0.15)+')';
  ctx.lineWidth=RING_SW+16; ctx.shadowColor=CLR.gold; ctx.shadowBlur=30+glow*16;
  ctx.beginPath(); ctx.arc(0,0,RING_R*p,0,Math.PI*2); ctx.stroke();
  // メイン
  ctx.strokeStyle=CLR.gold; ctx.lineWidth=RING_SW; ctx.shadowBlur=12;
  ctx.beginPath(); ctx.arc(0,0,RING_R,0,Math.PI*2); ctx.stroke();
  // 内側
  ctx.strokeStyle='#fff9c4'; ctx.lineWidth=2.5; ctx.shadowBlur=0;
  ctx.beginPath(); ctx.arc(0,0,RING_R-RING_SW*0.3,0,Math.PI*2); ctx.stroke();
  // 中心ドット
  ctx.fillStyle=CLR.gold; ctx.shadowColor=CLR.gold; ctx.shadowBlur=14;
  ctx.beginPath(); ctx.arc(0,0,6,0,Math.PI*2); ctx.fill();
  // TAP表示
  ctx.globalAlpha *= 0.7;
  ctx.fillStyle='#fffde0'; ctx.font='bold 14px system-ui'; ctx.textAlign='center';
  ctx.shadowBlur=6;
...
function drawShards(prog, isMiss) {
  var spread=prog*90, al=1-prog;
  for (var i=0;i<12;i++) {
    var a=(i/12)*Math.PI*2, d=spread*(0.5+(i%3)*0.25);
    var len=(10+(i%4)*5)*(1-prog*0.6);
    ctx.save();
    ctx.globalAlpha=al*al;
    ctx.translate(Math.cos(a)*d, Math.sin(a)*d);
    ctx.rotate(a);
    ctx.strokeStyle = isMiss ? (i%2===0?CLR.danger:'#ff8899') : (i%2===0?CLR.white:CLR.neon);
    ctx.lineWidth=2; ctx.shadowColor=isMiss?CLR.danger:CLR.neon; ctx.shadowBlur=10;
...
function drawBullet(b) {
  ...
  ctx.strokeStyle=g; ctx.lineWidth=5; ctx.shadowColor=CLR.neon; ctx.shadowBlur=16;
  ctx.beginPath(); ctx.moveTo(-tl,0); ctx.lineTo(0,0); ctx.stroke();
  ctx.fillStyle=CLR.white; ctx.shadowBlur=22;
...
function emitParticles(x, y, opts) {
  ...
  for (var i=0;i<c;i++) {
    ...
    particles.push({
      ...
      size:size*(0.5+Math.random()*0.5),shape:shape,drag:0.93
    });
  }
}
function emitBurst(x,y,opts){ emitParticles(x,y,Object.assign({count:26,speed:6,size:5,life:1.1},opts)); }
function emitSparks(x,y,opts){ emitParticles(x,y,Object.assign({count:18,speed:7,size:2,life:0.7,shape:'spark',color:CLR.white,glow:'rgba(255,255,255,0.8)'},opts)); }

function updateParticles(dt) {
  for (var i=particles.length-1;i>=0;i--) {
    var p=particles[i];
    p.x+=p.vx; p.y+=p.vy; p.vy+=0.2; p.vx*=p.drag; p.vy*=p.drag; p.life-=dt;
...
// ─── Utensils ─────────────────────────────────────────────────────────────────
var UCOLS = {
  spoon:{ fill:'#b0c8e8', stroke:'#7aaad4', glow:'rgba(140,190,230,0.7)' },
  fork: { fill:'#c8b0e8', stroke:'#9a7ad4', glow:'rgba(180,140,230,0.7)' },
  knife:{ fill:'#e8d0b0', stroke:'#d4a870', glow:'rgba(230,200,140,0.7)' }
};

function makeUtensil(type, wave) {
  // 上エリア全体に広く配置
  return {
    type:type, wave:wave, alive:true,
    x:30+Math.random()*330,
    y:BONUS_Y0+Math.random()*(BONUS_Y1-BONUS_Y0),
...
function updateUtensil(u, dt) {
  ...
  if(u.x<25)u.vx=Math.abs(u.vx)+0.3;
  if(u.x>365)u.vx=-Math.abs(u.vx)-0.3;
  if(u.y<BONUS_Y0+5)u.vy=Math.abs(u.vy)+0.3;
  if(u.y>BONUS_Y1-5)u.vy=-Math.abs(u.vy)-0.3;
}
...
function drawUtensil(u) {
  ...
  ctx.shadowColor=c.glow; ctx.shadowBlur=12+g*10;
...
function utensilHit(u, px, py) {
  return Math.hypot(px-u.x,py-u.y) < 32*u.scale;
}

function spawnWave1() {
  utensils=[];
  for(var i=0;i<16;i++) utensils.push(makeUtensil('spoon',1));
  utensilWave=1; waveLabel='🥄  SPOON BREAK!  ×2';
  bigMsgs.push(makeBig('SPOON BREAK!','タップで破壊！ ×2',CLR.neon,1.6));
  shakeDecay=4;
}

function spawnWave2() {
  utensils=[];
  for(var i=0;i<10;i++) utensils.push(makeUtensil('fork',2));
  for(var i=0;i<8;i++)  utensils.push(makeUtensil('knife',2));
  utensilWave=2; waveLabel='🍴  FORK & KNIFE!  ×4';
  bigMsgs.push(makeBig('WAVE 2!','フォーク＆ナイフ ×4','#c8b0e8',1.4));
  shakeDecay=5;
}
...
function glassBtn(x,y,w,h,color,pulse) {
  ...
  ctx.fillStyle='rgba('+rv+','+gv+','+bv+',0.14)';
...
// ─── Popups & Big Messages ────────────────────────────────────────────────────
function makePopup(x,y,text,color){ return {x:x,y:y,text:text,color:color||CLR.neon,life:1.0,vy:-1.8}; }
function updatePopup(p,dt){ p.y+=p.vy; p.life-=dt*1.8; }
function drawPopup(p) {
  if(p.life<=0)return;
  ctx.save(); ctx.globalAlpha=Math.min(1,p.life*2);
  ctx.fillStyle=p.color; ctx.font='bold 22px system-ui'; ctx.textAlign='center';
  ctx.shadowColor=p.color; ctx.shadowBlur=14;
  ctx.fillText(p.text,p.x,p.y); ctx.restore();
}
function makeBig(text,sub,color,dur){ return {text:text,sub:sub||'',color:color||CLR.neon,life:dur||1.0,maxLife:dur||1.0,scale:1.45}; }
function updateBig(m,dt){ m.life-=dt; m.scale=Math.max(1,m.scale-dt*3.2); }
function drawBig(m) {
  if(m.life<=0)return;
  ...
  ctx.shadowColor=m.color; ctx.shadowBlur=32;
  ctx.strokeStyle='rgba(0,0,0,0.65)'; ctx.lineWidth=7; ctx.lineJoin='round';
  ctx.strokeText(m.text,CW/2,CH*0.45); ctx.fillText(m.text,CW/2,CH*0.45);
  if(m.sub){
    ctx.font='600 18px system-ui'; ctx.shadowBlur=12;
    ctx.fillStyle='rgba(255,255,255,0.88)'; ctx.shadowColor='rgba(255,255,255,0.5)';
    ctx.fillText(m.sub,CW/2,CH*0.45+32);
  }
  ctx.restore();
}
...
function updateBg(dt) {
  ...
  if(shakeDecay>0){
    ...
    shakeDecay*=0.76; if(shakeDecay<0.3)shakeDecay=0;
  } else { shakeX=0; shakeY=0; }
  scanY=(scanY+1.5+bgUrgency*2.5)%CH;
...
function drawBg() {
  ...
  var bg=ctx.createRadialGradient(CW/2,CH*0.38,0,CW/2,CH/2,CH*0.8);
  ...
  if(bgUrgency>0){
    var vig=ctx.createRadialGradient(CW/2,CH/2,CH*0.15,CW/2,CH/2,CH*0.72);
    vig.addColorStop(0,'transparent'); vig.addColorStop(1,'rgba(255,30,60,'+(0.22*bgUrgency)+')');
    ctx.fillStyle=vig; ctx.fillRect(-shakeX,-shakeY,CW,CH);
  }
  // grid
  var pulse=(Math.sin(bgTime*3)+1)/2;
  var baseA=0.06+pulse*0.04*bgPulse+bgUrgency*0.05;
  var gcx=CW/2, gcy=CH*0.38;
  ...
  if(bgPulse>0){
    ...
    gl.addColorStop(0,'rgba(124,58,237,'+(0.28*bgPulse)+')'); gl.addColorStop(1,'transparent');
...
function drawZoneDivider() {
  if (utensilWave === 0) return;
  ctx.save();
  ctx.strokeStyle='rgba(0,212,255,0.2)';
  ctx.lineWidth=1; ctx.setLineDash([6,8]);
  ctx.beginPath(); ctx.moveTo(0,BONUS_Y1); ctx.lineTo(CW,BONUS_Y1); ctx.stroke();
  ctx.setLineDash([]);
  ctx.restore();
}
...
function drawHUD() {
  ...
  roundRect(ctx,0,0,CW,76,0); ctx.fill();
  ctx.strokeStyle=CLR.glassB; ctx.lineWidth=0.5;
  roundRect(ctx,0,0,CW,76,0); ctx.stroke();
  ...
  ctx.shadowColor=CLR.neon; ctx.shadowBlur=urgent?18:6;
  ...
  ctx.shadowColor=urgent?CLR.danger:CLR.neon; ctx.shadowBlur=urgent?22:6;
  ...
  if(combo>=2){
    ...
    ctx.shadowColor=combo>=5?CLR.goldG:CLR.neonG; ctx.shadowBlur=16;
  ...
  if(utensilWave>0&&waveLabel){
    ...
    ctx.fillText(waveLabel,CW/2,90); ctx.shadowBlur=0;
  }
  ctx.restore();
}
...
function drawStart() {
  ...
  var cx=CW/2, cy=CH*0.32, pulse=(Math.sin(bgTime*2)+1)/2;
  ctx.strokeStyle='rgba(0,212,255,'+(0.1+pulse*0.09)+')';
  ctx.lineWidth=30; ctx.shadowColor=CLR.neon; ctx.shadowBlur=32;
  ctx.beginPath(); ctx.arc(cx,cy,74,0.6,Math.PI*2-0.6); ctx.stroke();
  ctx.strokeStyle=CLR.neon; ctx.lineWidth=11; ctx.shadowBlur=20;
  ctx.beginPath(); ctx.arc(cx,cy,74,0.6,Math.PI*2-0.6); ctx.stroke();
  ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=2; ctx.shadowBlur=0;
  ctx.beginPath(); ctx.arc(cx,cy,64,0.6,Math.PI*2-0.6); ctx.stroke();

  ctx.textAlign='center';
  ctx.font='900 40px system-ui'; ctx.fillStyle=CLR.white;
  ctx.shadowColor=CLR.neon; ctx.shadowBlur=22;
  ctx.fillText('VISION',CW/2,cy+136);
  ctx.fillStyle=CLR.neon; ctx.shadowBlur=32;
  ctx.fillText('BREAK',CW/2,cy+184); ctx.shadowBlur=0;
  ctx.font='400 13px system-ui'; ctx.fillStyle=CLR.textM;
  ctx.fillText('近未来視覚トレーニング',CW/2,cy+212);
  if(bestScore>0){
    ctx.font='500 12px system-ui';
    ctx.fillText('BEST  '+String(bestScore).padStart(6,'0'),CW/2,cy+238);
  }
  var bx=CW/2-95, by=CH*0.72;
  glassBtn(bx,by,190,56,CLR.neon,pulse);
  ctx.fillStyle=CLR.white; ctx.font='700 20px system-ui'; ctx.textAlign='center';
  ctx.shadowColor=CLR.neon; ctx.shadowBlur=12;
  ctx.fillText('TAP TO START',CW/2,by+36); ctx.shadowBlur=0;
  ctx.font='400 11px system-ui'; ctx.fillStyle=CLR.textM;
  ctx.fillText('フリック→Cの向き ／ タップ→Oをタップ',CW/2,CH-26);
  ctx.restore();
}
...
function drawResult() {
  ...
  var pw=320,ph=450,px=(CW-pw)/2,py=(CH-ph)/2;
  glassPanel(px,py,pw,ph,24);

  ctx.textAlign='center'; ctx.font='900 34px system-ui';
  ctx.fillStyle=missed?CLR.danger:CLR.neon;
  ctx.shadowColor=missed?CLR.dangerG:CLR.neonG; ctx.shadowBlur=20;
  ctx.fillText(missed?'GAME OVER':'TIME UP!',cx,py+56); ctx.shadowBlur=0;

  if(score>=bestScore&&score>0){
    ctx.font='700 13px system-ui'; ctx.fillStyle=CLR.gold;
    ctx.shadowColor=CLR.goldG; ctx.shadowBlur=14;
  ...
  for(var i=0;i<stats.length;i++){
    var s=stats[i], ry=py+120+i*54;
    ctx.fillStyle=i%2===0?'rgba(255,255,255,0.03)':'transparent';
    ctx.fillRect(px+12,ry-18,pw-24,44);
    ctx.fillStyle=CLR.textM; ctx.font='500 11px system-ui'; ctx.textAlign='left';
  ...
  ctx.strokeStyle=CLR.glassB; ctx.lineWidth=0.5;
  ctx.beginPath(); ctx.moveTo(px+20,py+400); ctx.lineTo(px+pw-20,py+400); ctx.stroke();
  ctx.font='400 10px system-ui'; ctx.fillStyle=CLR.textM; ctx.textAlign='center';
  ctx.fillText('VISION BREAK  SCORE '+score+'  BREAK '+breakCount+'  #VISIONBREAK',cx,py+420);

  var bw=134,bh=52;
  var b1x=cx-bw-8, b2x=cx+8, bby=py+ph-68;
  glassBtn(b1x,bby,bw,bh,CLR.purple,pulse);
  ctx.fillStyle=CLR.white; ctx.font='700 17px system-ui'; ctx.textAlign='center';
  ctx.shadowColor=CLR.purple; ctx.shadowBlur=10;
  ctx.fillText('RETRY',b1x+bw/2,bby+34);
  glassBtn(b2x,bby,bw,bh,CLR.neon,pulse);
  ctx.fillStyle=CLR.white; ctx.shadowColor=CLR.neon; ctx.shadowBlur=10;
...
function spawnBonus(){
  // ボーナスOは上エリアに出現（スプーンと同じゾーン）
  var x=80+Math.random()*230;
  var y=BONUS_Y0+50+Math.random()*(BONUS_Y1-BONUS_Y0-80);
  rings.push(makeRing(x,y,null));
  bonusSpawned=true;
  bigMsgs.push(makeBig('BONUS O!','タップして破壊！',CLR.gold,1.2));
}
...
function hitRing(ring,fx,fy){
  ...
  popups.push(makePopup(ring.x,ring.y-24,'+'+pts,CLR.neon));
...
function hitBonus(ring){
  if(ring.hit||ring.state!=='alive')return;
  ring.hit=true; ring.state='breaking'; ring.breakP=0;
  bonusBreak++; score+=300; saveScore();
  emitBurst(ring.x,ring.y,{count:36,color:CLR.gold,glow:CLR.goldG,speed:9});
  emitSparks(ring.x,ring.y,{color:CLR.gold,glow:CLR.goldG});
  popups.push(makePopup(ring.x,ring.y-24,'+300 BONUS!',CLR.gold));
  shakeDecay=6;
  // 少し遅れてスプーン出現
  setTimeout(function(){
    if(GS==='playing') spawnWave1();
  },500);
}
...
function hitUtensil(u,px,py){
  ...
  emitParticles(px,py,{count:16,color:col,glow:'rgba(140,190,230,0.7)',speed:5,size:5});
  popups.push(makePopup(px,py,'+'+pts,col));
  // 全滅チェック
  if(utensils.every(function(v){return !v.alive;})){
    if(utensilWave===1){
      setTimeout(function(){
        if(GS==='playing') spawnWave2();
      },400);
    } else if(utensilWave===2){
      utensilWave=0; waveLabel='';
      bigMsgs.push(makeBig('PERFECT!','ボーナスクリア！',CLR.gold,1.4));
      shakeDecay=7;
    }
  }
}

function doMiss(wrongRing){
  if(GS!=='playing')return;
  // 失敗したリングを赤くしてフェードアウト
  if(wrongRing){
    wrongRing.missed=true;
    wrongRing.state='fading';
    wrongRing.fadeP=0;
    // 失敗跡（霊）を追加
    addMissGhost(wrongRing);
    // 他のリングもフェード
    for(var i=0;i<rings.length;i++){
      var r=rings[i];
      if(!r.hit&&r.state==='alive'){
        r.missed=true; r.state='fading'; r.fadeP=0;
        addMissGhost(r);
      }
    }
  }
  ...
  combo=0; missFlash=1; timeScale=0.12; shakeDecay=14;
  emitBurst(CW/2,CH/2,{count:32,color:CLR.danger,glow:CLR.dangerG,speed:10,size:7});
  emitSparks(CW/2,CH/2,{color:CLR.danger,glow:CLR.dangerG});
  setTimeout(function(){timeScale=1;saveScore();},700);
}
...
function onTap(x,y){
  ...
  for(var i=0;i<rings.length;i++){
    ...
    if(Math.hypot(x-r.x,y-r.y)<64*r.scale){hitBonus(r);return;}
  }

  // ② スプーン/フォーク/ナイフタップ（上エリア）
...
function onFlick(x,y,dir,dx,dy){
  ...
  for(var i=0;i<rings.length;i++){
    ...
    if(d<120&&d<bestD){best=r;bestD=d;}
  }

  if(!best){
    // フリック先にリングがない → スルー（ミスにしない）
    return;
  }
  if(dir===best.dir){
    hitRing(best,x,y);
...
function loop(ts){
  ...
  if(GS==='playing'){
    ...
    if(spawnTimer<=0){
      ...
      if(!bonusSpawned&&GS==='playing'){
        ...
        if(el2>8&&Math.random()<0.025)spawnBonus();
      }
      var iv=timeLeft>20?2.4:timeLeft>10?1.5:0.95;
      spawnTimer=iv/need;
    }
    // リング更新
    ...
    if(timeScale<1)timeScale=Math.min(1,timeScale+dt*1.8);
    if(missFlash>0)missFlash-=dt*2.8;
  }
  ...
  if(GS==='playing'||GS==='result'){
    ...
    // ミスフラッシュ + ガラス割れ
    if(missFlash>0){
      ctx.fillStyle='rgba(255,30,60,'+(missFlash*0.48)+')';
      ctx.fillRect(0,0,CW,CH);
      if(missFlash>0.4){
        ctx.save();
        var cf=(missFlash-0.4)/0.6;
        ctx.strokeStyle='rgba(255,80,110,'+(cf*0.7)+')';
        ctx.lineWidth=2;
        var ccx=CW/2,ccy=CH/2;
        // 放射状のひび割れ
        for(var i=0;i<12;i++){
          var a=(i/12)*Math.PI*2+(i%3)*0.08;
          var r1=20+Math.random()*30, r2=100+Math.random()*130;
          ctx.beginPath();
          ctx.moveTo(ccx+Math.cos(a)*r1, ccy+Math.sin(a)*r1);
          // 途中で枝分かれ
          var mid=r1+(r2-r1)*0.5;
          ctx.lineTo(ccx+Math.cos(a)*mid, ccy+Math.sin(a)*mid);
          ctx.lineTo(ccx+Math.cos(a+0.18)*r2, ccy+Math.sin(a+0.18)*r2);
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(ccx+Math.cos(a)*mid, ccy+Math.sin(a)*mid);
          ctx.lineTo(ccx+Math.cos(a-0.12)*r2, ccy+Math.sin(a-0.12)*r2);
          ctx.stroke();
        }
        ctx.restore();
      }
